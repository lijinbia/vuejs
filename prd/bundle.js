...
/***/ function(module, exports) {
	...
	module.exports = "var vue=require(\'../lib/vue.js\');var Swiper=require(\'../lib/swiper.js\');new vue({	el:\"#m-list\",	data:{		swiper:null,		nav:[		\"热门\",\"分类\",\"品牌\",\"珠宝\"		],		list:[],		navIndex:0	},	methods:{		changeTab:function(index){			this.navIndex=index;			this.swiper.slideTo(index);		}	},	mounted:function(){		fetch(\'/api/list\').then(response=>response.json())        .then(res=>{        	//console.log(res)        	var that=this;        	that.list=res;        	        	this.swiper=new Swiper(\'#index-swiper\',{        		loop:false,        		onSlideChangeStart:function(swiper){        		  that.navIndex = swiper.activeIndex	        		}        	})        })        .catch(e => console.log(\"error\",e))	}		})"
